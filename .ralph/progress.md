...
**NON FAIT — décision requise avant de continuer (cf. passation)** : le dossier de fonctions monté sur le conteneur `supabase-edge-functions-r11yqnmzzgv5qn8138xddwzt` (`/data/coolify/services/r11yqnmzzgv5qn8138xddwzt/volumes/functions`, accès `ls` refusé sans sudo) contient encore le code Saqr D'AVANT CE PROJET — noms de fonctions complètement différents du repo actuel (`reddit-collect` pas `scraper-reddit`, `llm-qualify-batch`+`llm-score-batch` pas `llm-score`, plus `nahda-bridge`/`youtube-ideas`/`watchlist-tick`/`topics-of-interest`/`generate-live-report`/`public-report`/`minio-init` absents du repo Kairos). **Rien du repo actuel n'est donc déployé sur .11**, y compris tout le travail des sessions précédentes (dispatch-llm péage, anti-injection, déterminisme, etc.) — seul le comportement DB (migrations, RLS, fonctions SQL) a été testé live jusqu'ici, jamais le code des edge functions elles-mêmes. `score-pending`/`slack-digest`/`cron-pipeline-trigger` EXISTENT déjà comme dossiers sur le serveur mais avec l'ANCIEN contenu Saqr (orphelins : aucun cron actif ne les appelle avec le bon nom de fonction SQL trigger, donc sans risque immédiat). Un déploiement (sync du dossier + restart du conteneur, potentiellement de tout le stack Coolify) est nécessaire pour que CE portage (et tout le reste) devienne réellement actif — **decision à prendre avec Amine** : sync complet (risque d'écraser des fonctions Saqr-only potentiellement utilisées par de vrais consommateurs Bassira/Nahda en prod) vs sync sélectif des fonctions homonymes seulement. Le service Coolify s'appelle `supabase-saqr`, confirmé partagé Saqr/Kairos/Bassira/Nahda — PAS un environnement de dev isolé.
