...
export const STAGE_TIMEOUTS_MS = {
  ...
  scrape: 90_000,
  // llm-score-batch : 30 signaux scorés en parallèle, chacun fait 2-3
  // appels LLM séparés (criteria + disqualifier-gate + soft-boost-gate),
  // soit 60-90 dispatch-llm calls en parallèle au pire — voir
  // scoring-engine.ts scoreSignalWithRubric. Le SLOWEST détermine le
  // wall-clock. 90s saturait dès qu'OpenRouter avait un pic (cf. session
  // 777f6c28 2026-05-17 : STAGE_TIMEOUT 90002ms pile à l'AbortSignal).
  // Bumpé à 150s 2026-05-17, même plafond que synthesize. Le gateway
  // Supabase Edge wall-clock est ~150s, on en reste sous.
  score: 150_000,
...
export interface RequestBody {
  ...
  /**
   * Token client (1-64 chars `[A-Za-z0-9_-]`) pour dedup retries.
   * Si fourni, le couple (api_key_id, idempotency_key) est unique. Un POST
   * dupliqué retourne la session existante (idempotent) au lieu d'en créer une 2e.
...
export function validateRequestBody(raw: unknown): BodyValidationResult {
  ...
  if (obj.idempotency_key !== undefined && obj.idempotency_key !== null) {
    ...
    if (!IDEMPOTENCY_KEY_RE.test(obj.idempotency_key)) {
      return { ok: false, error: 'idempotency_key_format_invalid' }
    }
...
/**
 * Empreinte courte du seed pour logs/observabilité — anti-PII leak.
 * SHA-256 hex, 16 premiers chars (64 bits, collision pratiquement impossible
 * sur des seeds business). Permet de grouper sans exposer le seed clair.
 * Hotfix 2026-05-17 — devil-advocate hardening.
 */
