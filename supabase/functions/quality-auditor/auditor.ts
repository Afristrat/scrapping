...
export const COVERAGE_FAIL_RATIO = 0.30;
export const LINGUISTIC_DOMINANCE = 0.90;
...
): { ratio: number; uncoveredSubjects: string[]; issues: AuditIssue[] } {
  ...
  if (ratio >= LINGUISTIC_DOMINANCE && dominant) {
    ...
    issues.push({
      ...
      description:
        ...
        `${expected.join("+")}. Langues absentes : ${
          missing.join(", ") || "aucune"
        }.`,
      fix_action: "trigger_deepening",
      auto_correction: null,
 ...
 */
export function checkBriefFormat(
  topics: TopicShape[],
): { issues: AuditIssue[]; corrections: Record<string, string> } {
  const issues: AuditIssue[] = [];
...
const LLM_SEVERITIES: ReadonlySet<IssueSeverity> = new Set([
  "high",
  "medium",
  "low",
]);
...
export function parseLlmIssues(raw: string): AuditIssue[] {
  ...
  for (const item of issues) {
    ...
    if (
      ...
      !LLM_SEVERITIES.has(severity as IssueSeverity)
    ) continue;
    if (typeof location !== "string" || location.length === 0) continue;
    if (typeof description !== "string" || description.length === 0) continue;
    const fixActionTyped: FixAction =
      LLM_FIX_ACTIONS.has(fixAction as FixAction)
    ...
    result.push({
      ...
      auto_correction: typeof autoCorrection === "string"
        ? autoCorrection
        : null,
    });
  }
...
export function deepeningTargetFromIssue(
  ...
  if (issue.axis === "coverage" && uncoveredSubjects.length > 0) {
    ...
    return {
      ...
      context: `Subjects sans aucun signal retenu : ${
        uncoveredSubjects.join(", ")
      }.`,
      suggested_sub_seed:
        `Recherche ciblée sur les sujets non couverts au 1er pass : ` +
        `${subjectTitles}.`,
    };
  }
...
export function computeVerdict(
  ...
  for (const issue of high) {
    ...
    if (t) {
      ...
      const dupe = targets.find(
        (x) => x.type === t.type && x.context === t.context,
      );
