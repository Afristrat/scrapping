...
[ETAT]
Branche `ralph/k06-orchestrator`, head `c85b802` poussé (PR #11 https://github.com/Afristrat/scrapping/pull/11, CONFLICTING vs main — assumé). Gates locales VERTES au dernier commit : deno test **468/468** · deno check 8/8 fns touchées · tsc 0 · lint 0 · build OK. CI GitHub ressuscitée (runs success sur 7177567/1cf756f/a34974a/9f76152/0a7cb48/4670b0e) ; le run sur `c85b802` était **in_progress** à la fin de session — vérifier son statut au réveil avant de continuer (`gh api "repos/Afristrat/scrapping/actions/runs?branch=ralph/k06-orchestrator&per_page=2"`). Base .11 (`db.saqr.ma`, stack Coolify `supabase-db-r11yqnmzzgv5qn8138xddwzt`) : migrations `20260511000001` + `20260512000001` APPLIQUÉES et prouvées live. **Migration `20260512000002` (cron-pipeline-trigger) NON APPLIQUÉE sur .11** — à faire avant tout test runtime du cron. `main` (b51841f) = ligne divergente Bassira NON mergée — décision Amine, ne pas auto-merger (checkout dans le worktree `claudia-zlatan-scrap-main`).

[FAIT] (tout prouvé par commande, 6 commits cette session : 7177567, 1cf756f, a34974a, 9f76152, 0a7cb48, 4670b0e, c85b802 — détail exhaustif dans `.ralph/progress.md`)

1. **CI morte → cause racine + fix** (`7177567`) : PR #11 `CONFLICTING` vs main → GitHub ne construit pas la ref de merge → triggers `pull_request` ne partent JAMAIS (83 runs historiques, tous `push` main). Fix : trigger `push` sur `ralph/**` + gate Deno élargie (minio seul → suite complète). Prouvé vert à chaque commit suivant.
2. **A#2 Topics par embeddings** (`1cf756f`) : `_shared/embeddings.ts` (fetchEmbeddingsBatch+chunking, cosineSimilarity, rankBySimilarity +6t, resolveEmbeddingKeys). enrich-signal (source='embedding', fallback LLM sans clé) + topic-classifier (LLM réservé aux signaux sans correspondance = nouveaux topics). ⚠ seuil 0.4 = knob NON calibré (ponytail annoté ×2) — à mesurer sur données réelles .11.
3. **A#3 Entités person en code + canonicalisation** (`a34974a`) : migration `20260512000001` (trigger DB `normalized_name` = autorité unique lower+unaccent+[a-z0-9], fusion doublons, index UNIQUE) — prouvée live (« Öpen AÏ »→openai, « open ai » absorbé). enrich-entities : person = auteur raw_payload en code (extractAuthor factorisé \_shared/signal-text.ts, digest délègue), LLM restreint org/tech/paper/product.
4. **A#4 Pré-filtre mécanique disqualifiers** (`9f76152`) : les règles réelles sont SÉMANTIQUES → pas de regex-NLP (disqualification à tort = DÉFCON 1) ; champ structuré optionnel `mechanical` (source_in|text_matches|older_than_days vs signal_date transporté). Matche → zéro appel LLM (criteria compris, coût 0) ; inévaluable → reversé au LLM. +8 tests. rubric-architect peut l'émettre.
5. **C#3 signal-synthesizer** (`0a7cb48`) : `freshness_median_days` LLM = hallucination pure (aucune date dans son input) → computeTopicProvenance + computeCulturalWarnings + mono_source réel EN CODE (+5 tests), prompt allégé (« NE CALCULE AUCUNE MÉTRIQUE »).
6. **Portage Saqr P1 — cron-pipeline-trigger** (`c85b802`, EN COURS — 1/4 fonctions du lot livrée) : pipeline 100 % manuel avant cette session (aucun cron ne déclenchait `run-pipeline`). Nouvelle fn `cron-pipeline-trigger` (protégée x-cron-secret constant-time, cible un user précis OU tous les `settings.cron_enabled=true`, retry 3x sur 5xx/429, trace `cron_last_run_at/status`). Migration `20260512000002` : colonnes cron\_\* sur settings + `unscored_signals_for(p_user_id, lim)` (variante SECURITY DEFINER de unscored_signals pour le mode interne, EXECUTE réservé service_role) + job pg_cron quotidien 05:00 UTC. **Toute la chaîne convertie en dual-mode ADR 0009** (resolveCaller/buildInternalHeaders/resolveOrgId — ce dernier factorisé depuis dispatch-llm vers `_shared/internal-auth.ts`) : run-pipeline, llm-score, topic-classifier, scraper-reddit/arxiv/x/rss — org_id désormais explicite sur tous leurs inserts/logs (les DEFAULT SQL reposent sur auth.uid(), inopérants en service_role).
7. Préexistants corrigés (règle n°3) : cluster-signals typait ses helpers `ReturnType<typeof createClient>` (10 erreurs deno check) → `SupabaseClient` (3e occurrence du pattern) ; const morte HAIKU_MODEL purgée ; 2 extractSignalText locaux divergents purgés (enrich-signal, enrich-entities) → canonique \_shared.

[ALERTE]

- **Migration `20260512000002` NON appliquée sur .11** — cron-pipeline-trigger et unscored_signals_for n'existent pas encore en base. L'appliquer AVANT tout test runtime (scp + `psql -v ON_ERROR_STOP=1 --single-transaction`).
- **Portage Saqr P1 INCOMPLET** : reste `score-pending` (chaîne de batchs auto-ré-invoquée — scoring cappé 50/run dans run-pipeline, backlog jamais rattrapé au-delà), `slack-digest` (zéro LLM), chaînon RSS Google News (~30 l, rss_keywords jamais exploités), + pépites (backoff 429 arXiv, welford.test.ts). Repo Saqr lecture seule : `C:\projets\Saqr` ; doc portage `docs/bridges/prompt-portage-saqr-vers-kairos.md`. Règles déjà rodées cette session : callLiteLLM→dispatch-llm, bassira-auth→internal-auth.ts (resolveCaller/buildInternalHeaders/resolveOrgId), mono-user→org_id explicite (les DEFAULT SQL ne marchent pas en service_role).
- **cron-pipeline-trigger n'est PAS DÉPLOYÉ** (edge fn créée en local, jamais `supabase functions deploy`) — nécessite aussi `--no-verify-jwt` (protection = x-cron-secret, pas JWT).
...
[NEXT] (reprendre EXACTEMENT ici — portage Saqr P1 à finir)

1. **Finir le portage Saqr P1** : `score-pending` (277 l. dans `C:\projets\Saqr\supabase\functions\score-pending\index.ts` — chaîne de batchs auto-ré-invoquée), `slack-digest` (189 l., zéro LLM), chaînon RSS Google News (~30 l., `research-from-seed/lib.ts` skippe toujours RSS malgré `rss_keywords`). Même méthode que cron-pipeline-trigger : lire la fn Saqr, adapter dual-mode + org_id, tests, gates, commit+push.
2. **Appliquer migration `20260512000002` sur .11** (scp + psql transaction unique) puis **déployer** `cron-pipeline-trigger` (`--no-verify-jwt`) + `run-pipeline`/`llm-score`/`topic-classifier`/scrapers (dual-mode modifié) + poser `INTERNAL_FN_SECRET` + tester le cron end-to-end (invoquer avec x-cron-secret, vérifier settings.cron_last_run_status).
3. **Runtime restant** : mapper `public_api_keys.proxy_user_id` + test e2e 2ᵉ saut (llm-score-batch→dispatch-llm interne) + repointer CLAUDE.md + gen types --db-url propre + calibrer le seuil embeddings 0.4.
4. Reste audit : P1-007 sièges bornés, P1-008 Error Boundary (flag tâche #11 « completed » à re-vérifier — prd-blindage 008 encore passes=false), P2 (SSRF validate-api-key, run-pipeline lock, record-usage idempotent, workers atomiques, cascade non plafonnée), lockfiles, npm audit.

[CTX]
Accès .11 : `ssh -i ~/.ssh/serveurai_mnemo serveuria@192.168.100.11`. Base : `docker exec supabase-db-r11yqnmzzgv5qn8138xddwzt psql -U postgres`. Migrations serveur : `/home/serveuria/kairos-migrations/` (scp puis `psql -v ON_ERROR_STOP=1 --single-transaction < fichier`). Tests : `deno test --allow-env --node-modules-dir=auto supabase/functions/` (468) ; après deno → `bun install` avant tsc. CI : `gh api "repos/Afristrat/scrapping/actions/runs?branch=ralph/k06-orchestrator&per_page=3"`. Backup associé : `/home/serveuria/kairos-r11y-BEFORE-RESET.sql`. Docs L99 : `docs/audit/2026-07-07-l99-optimisation.md`. Repo Saqr lecture seule : `C:\projets\Saqr` (fns restantes : score-pending, slack-digest, scraper-rss RSS Google News). Piège outil Bash : le hook anti-fuite bloque les heredocs contenant des motifs type "Authorization"/secrets multi-lignes → passer par un script Python écrit via l'outil Write puis exécuté, jamais de heredoc inline pour ces patches.

[MEMO]
Déterministe ≠ LLM (fil rouge L99). Purger, ne pas overrider. « Fait » = re-vérifié à l'instant. Jamais afficher un secret. **Pin `claude-fable-5` — le poste a affiché un rappel de bascule opusplan en session ; Amine a déjà tapé `/model` → Sonnet 5 pendant cette session, donc la bascule semble déjà traitée côté modèle par défaut. Ne pas re-proposer `switch-to-opusplan.ps1` sans vérifier l'état réel du pin.**
